<template>
  ...
  <nav :class="{ light: isLightMode }"
    class="fixed select-none h-full border-r border-1 lg:border-gray-600 flex flex-col justify-center gap-4 transition-linear duration-500">
    <div class="w-full mt-8 absolute top-0">
      <div class="flex text-white w-full mx-auto">
        <div class="hidden lg:flex mx-14">
    ...
    <div v-for="(tab, index) in side_bar_tabs" :key="index" class="hidden lg:block">
      <div class="pl-6 pr-8 hidden lg:block">
        <div
          class="mx-auto rounded-xl text-xl text-white hover:bg-matcha hover:text-hoverText dark:hover:text-hoverText transition duration-300 cursor-pointer"
          ...
          <router-link :to="tab.route" class="flex flex-row py-3 w-full px-5 md:px-10 items-center gap-4">
            ...
            <div class="hidden sm:hidden md:block min-w-[130px]" :class="tab.active ? 'font-bold' : 'font-normal'">
              {{ tab.displayName }}
            </div>
          </router-link>
        </div>
    ...
    <div class="absolute bottom-0 mx-4 pr-8 w-full">
      <div
        class="text-white hover:text-hoverText flex flex-row mx-4 my-4 rounded-lg py-3 text-xl px-8 md:px-10 items-center gap-3 hover:bg-matcha  transition duration-300 cursor-pointer"
  ...
  <!-- nav bar for mobile-->
  <div
    class="flex lg:hidden h-[50px] bg-matcha border-t border-1 border-gray-600 fixed z-10 justify-between items-center bottom-0 w-full p-1 transition-ease-in-out duration-300">
        ...
        <div class="flex justify-center items-center p-1 cursor-pointer" @click="toggleButton(index)">
          <router-link :to="tab.route" class="flex flex-row py-3 w-full px-5 md:px-10 items-center gap-3">
            <component :is="tab.active ? tab.icon : tab.inactiveIcon"
              :class="['mobile_icon', tab.active ? 'text-hoverText' : 'text-hoverText']" />
          </router-link>
        </div>
      </div>
    </div>
  ...
  <main class="main_content transition-linear duration-500 h-full  lg:ml-[310px]">
